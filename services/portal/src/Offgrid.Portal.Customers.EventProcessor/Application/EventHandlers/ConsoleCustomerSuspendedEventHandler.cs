...
public sealed class ConsoleCustomerSuspendedEventHandler : IEventHandler<CustomerSuspendedEvent>
{
    public Task<bool> HandleAsync(CustomerSuspendedEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        AnsiConsole.Write(new Rule($"[white]Customer Suspended Event Received[/] [dim]{DateTime.Now:HH:mm:ss}[/]") { Style = Style.Parse("magenta") });

        var table = new Table().Border(TableBorder.Rounded).BorderColor(Color.Magenta);
